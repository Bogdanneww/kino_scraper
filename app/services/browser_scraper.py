...
async def scrape_movie_details(query: str) -> dict:
    async with async_playwright() as p:
        ...
        try:
            await page.goto(f"{settings.BASE_URL}/search/?q={query}", wait_until="networkidle")
            await page.locator(".item.search .title a").first.click()
            await page.wait_for_load_state("networkidle")

            return {
                "title": await page.get_attribute('meta[property="og:title"]', "content"),
                "year": _safe_int(await page.locator(".film-page__title-section .year").first.inner_text()
                                  if await page.locator(".film-page__title-section .year").count() else None),
                "rating": _safe_float(await page.get_attribute('meta[itemprop="ratingValue"]', "content")
                                      if await page.locator('meta[itemprop="ratingValue"]').count() else None),
                ...
                "poster": await page.get_attribute('meta[property="og:image"]', "content"),
                "url": page.url,
            }
        finally:
            await browser.close()
